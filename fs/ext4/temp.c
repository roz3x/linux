...
static struct ext4_range_node* find_first_le(u64 inode, u64 offset) {
...
static void insert_exact(struct ext4_range_node *new_node){
	struct rb_node **new = &root.rb_node;
	struct rb_node *parent = NULL;
	u64 offset = new_node->offset;
	u64 inode = new_node->inode;

	printk("[shivang] inserting %llx\n", offset);
...
static void delete_exact(u64 inode, u64 offset) {
...
void ext4_completion_cb(u64 inode, u64 start_offset, u64 end_offset) {

	mutex_lock(&lock);
	printk("[shivang] delete request :  %llx %llx\n", start_offset, end_offset);

	printk("[shivang] tree_size : %d\n", atomic_read(&tree_size));
	delete_exact(inode, start_offset);
	printk("[shivang] tree_size : %d\n", atomic_read(&tree_size));
	delete_exact(inode, end_offset);

	mutex_unlock(&lock);
}


void ext4_insert_or_wait(u64 inode, u64 start_offset,
	u64 end_offset) {
	...
	printk("[shivang] tree_size : %d %llx %llx\n", atomic_read(&tree_size), start_offset, end_offset);
